...
import { executeRepay } from '@/lib/transactions';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await executeRepay(userAddress, tokenAddress, amount, interestRateMode);
    ...
    return NextResponse.json({
      ...
      to: result.to,
      data: result.data,
      message: 'Repay transaction prepared. Sign with your wallet to execute.',
    });
  } catch (error: any) {
    console.error('[v0] Repay error:', error);
    return NextResponse.json(
      { error: error.message || 'Failed to prepare repayment' },
      { status: 500 }
    );
  }
}
